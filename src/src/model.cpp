      ...
      if (name == "nd") {
        ...
        if (category == "railway") {
          ...
          m_Buildings.back().outer = {way_num};
                   ...
                   (category == "natural" &&
                    (type == "wood" || type == "tree_row" || type == "scrub" ||
                     type == "grassland")) ||
...
static bool TrackRec(const std::vector<int> &open_ways, const Model::Way *ways,
  ...
  if (nodes.empty()) {
    for (int i = 0; i < open_ways.size(); ++i)
      if (!used[i]) {
        ...
        nodes = way_nodes;
        if (TrackRec(open_ways, ways, used, nodes)) return true;
        nodes.clear();
        used[i] = false;
      }
    return false;
    ...
    if (head == tail && nodes.size() > 1) return true;
    for (int i = 0; i < open_ways.size(); ++i)
      if (!used[i]) {
        const auto &way_nodes = ways[open_ways[i]].nodes;
...
static std::vector<int> Track(std::vector<int> &open_ways,
